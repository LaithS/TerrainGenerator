...
{
	PolygonizeToTriangles(WorkerTask->TerrainGrid, WorkerTask->SurfaceCrossValue, WorkerTask->ChunkStartSize, WorkerTask->ChunkEndSize, &WorkerTask->Vertices, &WorkerTask->Indices, &WorkerTask->Positions, WorkerTask->ChunkLocation, WorkerTask->ChunkScale, WorkerTask->TerrainTransform);

	return true;
}

int FProceduralTerrainWorker::PolygonizeToTriangles(UTerrainGrid *TerrainGrid, float p_fSurfaceCrossValue, FIntVector ChunkStartSize, FIntVector ChunkEndSize, TArray<FDynamicMeshVertex> *Vertices, TArray<int32> *Indices, TArray<FVector> *Positions, FVector &ChunkLocation, FVector &ChunkScale, FTransform &TerrainTransform)
{
	// TODO?
